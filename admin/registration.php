...
if (isset($_POST['submit'])) {
    ...
    $result = "SELECT count(*) FROM userRegistration WHERE email=? || enrno=?";
    $stmt = $mysqli->prepare($result);
    $stmt->bind_param('ss', $email, $enrno);
    $stmt->execute();
    $stmt->bind_result($count);
    $stmt->fetch();
    $stmt->close();

    // TODO: remove 100 th time execution to 0
    if ($count > 100){
    // if ($count > 0) {
        echo "<script>alert('Registration number or email id already registered.');</script>";
    } else {

        $query = "insert into  registration(roomno,seater,stayfrom,duration,course,enrno,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $stmt = $mysqli->prepare($query);
        $rc = $stmt->bind_param('iisisissssisississsisssi', $roomno, $seater, $stayfrom, $duration, $course, $enrno, $fname, $mname, $lname, $gender, $contactno, $emailid, $emcntno, $gurname, $gurrelation, $gurcntno, $caddress, $ccity, $cstate, $cpincode, $paddress, $pcity, $pstate, $ppincode);
        $stmt->execute();
        $stmt->close();

        $query1 = "insert into  userregistration(enrno,firstName,middleName,lastName,gender,contactNo,email,password) values(?,?,?,?,?,?,?,?)";
        $stmt1 = $mysqli->prepare($query1);
        $stmt1->bind_param('sssssiss', $enrno, $fname, $mname, $lname, $gender, $contactno, $emailid, $contactno);
        $stmt1->execute();
        echo "<script>alert('Student Succssfully register');</script>";

        $query2 = "insert into hostel_receipts (stu_name, receiptno) values(?,?)";
        $stmt2 = $mysqli->prepare($query2);
        $full_name = $fname.' '.$mname.' '.$lname;
        $stmt2->bind_param('ss', $full_name, $receiptno);
        $stmt2->execute();
        echo "<script>alert('Student receipt registered');</script>";
    }
}
?>
                                            ...
                                            <form method="post" action="" class="form-horizontal">
                                                ...
                                                <div class="form-group">
                                                    <label class="col-sm-4 control-label">
                                                        <h4 style="color: green" align="left">Room Related info
                                                        </h4>
                                                    </label>
                                                </div>

                                                <div class="form-group">
                                                    <label class="col-sm-2 control-label">Room no.
                                                    </label>
                                                    <div class="col-sm-8">
...
while ($row = $res->fetch_object()) {
    ?>
                                                ...
                                                <div class="form-group">
                                                    <label class="col-sm-2 control-label">Receipt No :
                                                    </label>
                                                    <div class="col-sm-8">
                                                        <!-- TODO: create a function for receiptno same as checkenrnoAvailability -->
                                                        <input type="text" name="receiptno" id="receiptno" class="form-control" required="required" onblur="checkenrnoAvailability()">
                                                ...
                                                <div class="form-group">
                                                    <label class="col-sm-2 control-label">
                                                        <h4 style="color: green" align="left">Personal info
                                                        </h4>
                                                    </label>
                                                </div>
...
while ($row = $res->fetch_object()) {
    ?>
                                                ...
                                                <div class="form-group">
                                                    <label class="col-sm-2 control-label">Enrollment No :
                                                    </label>
                                                    <div class="col-sm-8">
                                                        <input type="text" name="enrno" id="enrno" class="form-control" required="required" onblur="checkenrnoAvailability()">
                                                        <span id="user-reg-availability" style="font-size:12px;"></span>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    <label class="col-sm-3 control-label">
                                                        <h4 style="color: green" align="left">Correspondense Address
                                                        </h4>
                                                    </label>
                                                </div>
...
while ($row = $res->fetch_object()) {
    ?>
                                                ...
                                                <div class="form-group">
                                                    <label class="col-sm-3 control-label">
                                                        <h4 style="color: green" align="left">Permanent Address
                                                        </h4>
                                                    </label>
                                                </div>
...
});</script><script>function checkAvailability() {
    $("#loaderIcon").show();
    jQuery.ajax({
...
}</script><script>function checkenrnoAvailability() {

    $("#loaderIcon").show();
    jQuery.ajax({
        url: "check_availability.php",
        data: 'enrno=' + $("#enrno").val(),
        type: "POST",
        success: function (data) {
